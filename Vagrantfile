...
Vagrant.configure("2") do |config|
  ...
  # Explicitly select the bridged interface so NIC selection is deterministic
  # across hosts (avoids interactive prompts / inconsistent adapters when the
  # host has multiple NICs such as VPN or virtual adapters).
  # Override per host with: VAGRANT_BRIDGE="your-interface-name" vagrant up
  bridge_interface = ENV.fetch("VAGRANT_BRIDGE", "eth0")
  ...
  config.vm.define "ansible_tower" do |ansible_tower|
    ...
    ansible_tower.vm.network "public_network", bridge: bridge_interface
  ...
  config.vm.define "deploy_env1" do |deploy_env1|
    ...
    deploy_env1.vm.network "public_network", bridge: bridge_interface
  ...
  config.vm.define "deploy_env2" do |deploy_env2|
    ...
    deploy_env2.vm.network "public_network", bridge: bridge_interface
